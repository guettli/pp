...
export async function loadPhonemeModel(options: LoadModelOptions = {}): Promise<{
    ...
    // Determine paths
    const modelPath = customModelPath || path.resolve(__dirname, '../../onnx/zipa-small-ctc-onnx-2026-01-28/model.onnx');
    const vocabPath = customVocabPath || path.resolve(__dirname, '../../onnx/zipa-small-ctc-onnx-2026-01-28/vocab.json');
...
export async function extractPhonemes(
    ...
    const nonSpecialTokens = grouped.filter(g =>
        g.tokenId !== 0 &&
        g.symbol !== '<blk>' &&
        g.symbol !== '▁'
    );
