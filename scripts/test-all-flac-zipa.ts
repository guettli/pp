 ...
 * Uses the int8 ONNX model.
 *
 * Usage: ./run tsx scripts/test-all-flac-zipa.ts [options] [pattern]
 *
 * Options:
...
const LOCAL_MODEL = path.join(CACHE_DIR, `${MODEL_NAME}.int8.onnx`);
...
async function downloadModelFiles(): Promise<{ modelPath: string; vocabPath: string }> {
  const modelPath = fs.existsSync(LOCAL_MODEL)
    ? LOCAL_MODEL
    : await downloadIfNeeded(MODEL_URL, `${MODEL_NAME}.int8.onnx`);
...
function printHelp(): void {
  console.log(`Usage: ./run tsx scripts/test-all-flac-zipa.ts [options] [pattern]
...
Examples:
  ./run tsx scripts/test-all-flac-zipa.ts              # Test all FLAC files
  ./run tsx scripts/test-all-flac-zipa.ts --update     # Update all YAML files
  ./run tsx scripts/test-all-flac-zipa.ts Wasser       # Test "Wasser" only
  ./run tsx scripts/test-all-flac-zipa.ts "Sch*"       # Test matching "Sch*"
`);
}
